...
const Schedule = () => {
  ...
  const fetchTasks = async () => {
    try {
      const data = await scheduleService.getTasks(viewYear, viewMonth + 1);
  ...
  const fetchNotes = async (taskId) => {
    try {
      const data = await scheduleService.getTaskNotes(taskId);
  ...
  const handleMasterSave = async () => {
    ...
    try {
      ...
      try {
        await scheduleService.bulkSaveTasks({
          year: viewYear,
          month: viewMonth + 1,
  ...
  const handleAddNote = async (e) => {
    ...
    setSubmitting(true);
    try {
      await scheduleService.updateTaskNotes(selectedTask.id, noteText);
      setNoteText("");
