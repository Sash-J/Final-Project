...
void mainImage(out vec4 o, vec2 C){
  ...
  vec2 cardCoord = C + uCardOffset;
  vec2 res = max(uCardResolution, vec2(1.0));
  vec2 uv = cardCoord / res;
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uCardOffset: { value: new Float32Array([0, 0]) },
        uCardResolution: { value: new Float32Array([1, 1]) },
      },
    });
    ...
    const setSize = () => {
      ...
      // Force canvas to stretch using CSS to eliminate sub-pixel gaps between adjacent containers
      ...
      const card = container.closest(".composite-bento-card");
      if (card) {
        const cardRect = card.getBoundingClientRect();
        const dpr = Math.min(window.devicePixelRatio || 1, 2);
        program.uniforms.uCardResolution.value[0] = cardRect.width * dpr;
        program.uniforms.uCardResolution.value[1] = cardRect.height * dpr;
        program.uniforms.uCardOffset.value[0] =
          (rect.left - cardRect.left) * dpr;
        program.uniforms.uCardOffset.value[1] =
          (cardRect.bottom - rect.bottom) * dpr;
      } else {
        program.uniforms.uCardResolution.value[0] = res[0];
        program.uniforms.uCardResolution.value[1] = res[1];
        program.uniforms.uCardOffset.value[0] = 0;
        program.uniforms.uCardOffset.value[1] = 0;
      }
