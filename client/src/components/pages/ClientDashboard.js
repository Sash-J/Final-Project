...
const ClientDashboard = () => {
    ...
    const toggleProjectStatus = async (project) => {
        ...
        try {
            await axios.put(`${API}/api/projects/${project.id}/status`, { status: newStatus }, { withCredentials: true });
    ...
    const fetchBudgetData = async (projectId, versionId) => {
        try {
            ...
            const values = valRes.data; // Backend returns dict keyed by budget_item_id
            ...
            {projects.length === 0 ? (
                ...
                <div className="cd-projects-grid">
                    {projects.map(project => {
                        ...
                        return (
                            <div key={project.id} className={`cd-project-card ${isCompleted ? 'cd-project-completed' : ''}`}>
                                <div className="cd-card-header">
                                    <div className="cd-card-header-inner">
                                        <h3>{project.project_name}</h3>
                                        <div className="cd-status-toggle">
                                            <span className={`cd-status-label ${isCompleted ? 'completed' : ''}`}>
                                                {isCompleted ? 'Completed' : 'In Progress'}
                                            </span>
                                            <label 
                                                className="cd-switch" 
                                                onClick={(e) => { e.preventDefault(); toggleProjectStatus(project); }}
                                            >
                                                <input 
                                                    type="checkbox" 
                                                    checked={isCompleted} 
                                                    readOnly
                                                />
                                                <span className={`cd-slider circular ${isCompleted ? 'completed' : ''}`}>
                                                    <span className={`cd-knob ${isCompleted ? 'completed' : ''}`}></span>
                                                </span>
                                            </label>
                                        </div>
                                    </div>
                                    {(!project.start_date || !project.end_date) && (
                                        <span className="cd-date">Started: {new Date(project.created_at).toLocaleDateString()}</span>
                                    )}
                                </div>
                                
                                {!isCompleted && (
                                    <div className="cd-mini-timeline-preview">
                                        <div className="cd-mini-timeline-preview-inner">
                                            <SuiTimeline projectId={project.id} userRole={user.role} preview={true} updateTrigger={milestoneUpdateTrigger} />
                                        </div>
                                    </div>
                                )}
                                ...
                                <div className="cd-card-actions">
                                    <button className="cd-view-budget-btn" onClick={() => setSelectedTimelineProject(project)} style={{ background: 'linear-gradient(135deg, #4facfe, #00f2fe)', marginRight: '10px' }}>
                                        View Full Timeline
                                    </button>
                                    <button className="cd-view-budget-btn" onClick={() => openBudgetModal(project)}>
                                        View Budget Sheet
                                    </button>
                                </div>
                            </div>
            ...
            {selectedProject && (
                <div className="cd-modal-overlay" onClick={closeBudgetModal}>
                    <div className="cd-modal-content" onClick={e => e.stopPropagation()}>
                        <button className="cd-modal-close" onClick={closeBudgetModal}>&times;</button>
                        ...
                        {budgetLoading ? (
                            <div className="cd-loading cd-modal-loading">Loading budget...</div>
                            ...
                            <div className="cd-empty cd-modal-empty">No filled budget rows found for this project.</div>
                        )}
                    </div>
                </div>
            )}
                    ...
                    >
                        ...
                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                            ...
                            <div style={{ background: 'rgba(0,0,0,0.2)', padding: '0', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)', flex: 1, overflow: 'hidden', position: 'relative' }}>
                                <SuiTimeline projectId={selectedTimelineProject.id} key={selectedTimelineProject._t || '1'} userRole={user.role} updateTrigger={milestoneUpdateTrigger} onMilestonesChange={() => setMilestoneUpdateTrigger(prev => prev + 1)} />
                            </div>
                        </div>
